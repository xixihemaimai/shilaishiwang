...
#define kTextColor [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1]
#define kDetailTextColor [UIColor colorWithRed:136/255.0 green:136/255.0 blue:136/255.0 alpha:1]
#define kSeparatorColor [UIColor colorWithRed:205/255.0 green:205/255.0 blue:205/255.0 alpha:1]
#define kCellBgColor [UIColor colorWithRed:245/255.0 green:245/255.0 blue:245/255.0 alpha:1]
#define kTextSelectColor [UIColor colorWithRed:51/255.0 green:133/255.0 blue:255/255.0 alpha:1]
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height andTag:(NSInteger)tag{
    //return [self initWithOrigin:origin width:[UIScreen mainScreen].bounds.size.width - 70 andHeight:height];
 
    
    
    return [self initWithOrigin:origin width:[UIScreen mainScreen].bounds.size.width - 115  andHeight:height andTag:tag];
}

- (instancetype)initWithOrigin:(CGPoint)origin width:(CGFloat)width andHeight:(CGFloat)height andTag:(NSInteger)tag{
    ...
    if (self) {
        ...
        _indicatorAlignType = DOPIndicatorAlignTypeCloseToTitle;
        
        self.remainMenuTitle = true;
        CGSize dropDownViewSize = CGSizeMake(_dropDownViewWidth, [UIScreen mainScreen].bounds.size.height);
        //lefttableView init
        
        
        //origin.x    self.frame.origin.y + self.frame.size.height  dropDownViewSize.width/2
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 1, SCREEN_WIDTH, 0) style:UITableViewStylePlain];
        ...
        //origin.x dropDownViewSize.width
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.frame.origin.y + self.frame.size.height,SCREEN_WIDTH , kButtomImageViewHeight)];
        _buttomImageView.image = [UIImage imageNamed:@"icon_chose_bottom"];
        
        //self tapped
        self.backgroundColor = [UIColor colorWithHexColorStr:@"#f0f0f0"];
        ...
        _backGroundView.backgroundColor = [UIColor colorWithHexColorStr:@"#f0f0f0" alpha:1.0];
...
//        [_backGroundView addGestureRecognizer:gesture];
       ...
       // bottomShadow.backgroundColor = kSeparatorColor;
        bottomShadow.backgroundColor = [UIColor colorWithHexColorStr:@"#f0f0f0"];
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        if (self.ishideMenuBlock) {
            self.ishideMenuBlock();
        }
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (haveItems) {
            ...
            //self.origin.x  self.frame.origin.y + self.frame.size.height _dropDownViewWidth/2
            
            _leftTableView.frame = CGRectMake(0, 1,SCREEN_WIDTH , 0);
            ...
            [self.backGroundView addSubview:_leftTableView];
            ...
            //self.origin.x   self.frame.origin.y + self.frame.size.height _dropDownViewWidth
            _leftTableView.frame = CGRectMake(0,1 , SCREEN_WIDTH, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + _dropDownViewWidth/2, self.frame.origin.y + self.frame.size.height, _dropDownViewWidth/2, 0);
            [self.backGroundView addSubview:_leftTableView];
        ...
        if (_showBottomImage) {
            //self.origin.x _dropDownViewWidth
            _buttomImageView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH, kButtomImageViewHeight);
            [self.backGroundView addSubview:_buttomImageView];
        }
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                //self.origin.x  self.frame.origin.y + self.frame.size.height _dropDownViewWidth/2
                _leftTableView.frame = CGRectMake(0,1, SCREEN_WIDTH, tableViewHeight);
            ...
            if (_showBottomImage) {
                //self.origin.x _dropDownViewWidth
                _buttomImageView.frame = CGRectMake(0, CGRectGetMaxY(_leftTableView.frame)-2,SCREEN_WIDTH , kButtomImageViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                ...
                //self.origin.xself.frame.origin.y + self.frame.size.height _dropDownViewWidth
                _leftTableView.frame = CGRectMake(0, 1, SCREEN_WIDTH, 0);
            }
            if (_showBottomImage) {
                //self.origin.x _dropDownViewWidth
