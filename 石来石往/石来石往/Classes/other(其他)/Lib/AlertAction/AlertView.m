...
static NSString * kPopoverHeaderdentifier = @"kPopoverHeaderdentifier";
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    AlertTableViewHeaderFootView * alertTableHeaderview = [[AlertTableViewHeaderFootView alloc]initWithReuseIdentifier:kPopoverHeaderdentifier];
    
     [alertTableHeaderview.editHeaderBtn setTitle:@"编辑" forState:UIControlStateNormal];
    [alertTableHeaderview.editHeaderBtn addTarget:self action:@selector(editUserContactAndAddress:) forControlEvents:UIControlEventTouchUpInside];
  //  alertTableHeaderview.contentView.backgroundColor = [UIColor colorWithHexColorStr:@"#fffffff"];
    if (self.isCurrent) {
        alertTableHeaderview.editHeaderBtn.hidden = NO;
    }else{
        alertTableHeaderview.editHeaderBtn.hidden = YES;
        
    }
    
    if ([_selectype isEqualToString:@"1"]) {
         alertTableHeaderview.titleLabel.text = @"联系人";
       
    }else if([_selectype isEqualToString:@"2"]){
         alertTableHeaderview.titleLabel.text = @"联系地址";
    }
   
    
    return alertTableHeaderview;
}

//编辑联系人和地址
- (void)editUserContactAndAddress:(UIButton *)editBtn{
    //传递是1还是2
    if ([self.delegate respondsToSelector:@selector(selectChangeContactsFuntionSelectype:andAlertView:)]) {
        [self.delegate selectChangeContactsFuntionSelectype:self.selectype andAlertView:self];
    }
}


static NSString * kPopoverFooterdentifier = @"kPopoverFooterdentifier";
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    AlertTableViewHeaderFootView * alertTableFooterview = [[AlertTableViewHeaderFootView alloc]initWithReuseIdentifier:kPopoverFooterdentifier];
    alertTableFooterview.titleLabel.text = @"取消";
    alertTableFooterview.userInteractionEnabled = YES;
    UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(hideCurrentView:)];
    [alertTableFooterview addGestureRecognizer:tap];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        cell = [[AlertViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:kPopoverCellIdentifier];
    }
    //[indexPath.section]
    //[cell setAction:_actionArr[indexPath.row]];
    cell.action = _actionArr[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        AlertAction *action = _actionArr[indexPath.row];
