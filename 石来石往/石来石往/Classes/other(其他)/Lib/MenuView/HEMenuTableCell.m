...
//            imageView.backgroundColor = [UIColor yellowColor];
            imageView.image = [UIImage imageNamed:@"打勾-1"];
            imageView.contentMode = UIViewContentModeScaleAspectFill;
            imageView.clipsToBounds = YES;
            imageView.frame = CGRectMake(CGRectGetMaxX(self.conLabel.frame), _height/2 - 3.5, 9, 7);
            imageView;
        });
    }
    return _iconImg;
...
- (UILabel *)conLabel
{
    if (!_conLabel) {
        self.conLabel = ({
            ...
            label.frame = CGRectMake(_width /2  - (_width - kImgLabelWidth - kLineXY - kImgWidth - 15)/2 , 0, _width - kImgLabelWidth - kLineXY - kImgWidth - 15, _height);
            label;
        });
    }
    return _conLabel;
