                      ...
                      delegate:(id<YBPopupMenuDelegate>)delegate
{
    if (self = [super init]) {
        ...
        _fontSize = 16;
        _textColor = [UIColor colorWithHexColorStr:@"#ffffff"];
        _offset = 0.0;
        _type = YBPopupMenuTypeDefault;
        _contentColor = [UIColor colorWithHexColorStr:@"#434343"];
        _separatorColor = [UIColor colorWithHexColorStr:@"#333333" alpha:0.6];
       // _isCount = false;
        ...
        _contentView.height -= 2 * KArrowHeight - 10;
        _contentView.centerY = _mainView.centerY;
        
        [_mainView addSubview: _contentView];
        [self addSubview: _mainView];
//        if (tag == 0) {
//            UIView * topHeaderView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _mainView.bounds.size.width, 39)];
//            _contentView.tableHeaderView = topHeaderView;
//
//            UILabel * label = [[UILabel alloc]initWithFrame:CGRectMake(12, 0, _mainView.bounds.size.width, 38)];
//            label.text = @"功能直达";
...
//
//            UIView * bottomview = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(label.frame), _mainView.bounds.size.width, 0.5)];
...
{
    YBPopupMenu *popupMenu = [[YBPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth andTag:1 delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth andTag:(NSInteger)tag delegate:(id<YBPopupMenuDelegate>)delegate
{
    YBPopupMenu *popupMenu = [[YBPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemWidth andTag:tag delegate:delegate];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.separatorColor = [UIColor colorWithHexColorStr:@"#EEEEEE"];
    
    
    
    UILabel * redLabel = [[UILabel alloc]init];
    redLabel.text = @"11";
    redLabel.font = [UIFont systemFontOfSize:9];
    redLabel.textAlignment = NSTextAlignmentCenter;
    redLabel.backgroundColor = [UIColor colorWithHexColorStr:@"#FF4B33"];
    redLabel.textColor = [UIColor colorWithHexColorStr:@"#FFFFFF"];
    [cell.contentView addSubview:redLabel];
    
    redLabel.sd_layout
    .leftSpaceToView(cell.contentView, 22)
    .topSpaceToView(cell.contentView, 7)
    .widthIs(16)
    .heightIs(12);
    ...
    if ([_titles[indexPath.row] isEqualToString:@"消息"]) {
       NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
       BOOL isMessage = [user objectForKey:@"isMessage"];
        if (isMessage) {
            redLabel.hidden = YES;
        }else{
            redLabel.hidden = NO;
