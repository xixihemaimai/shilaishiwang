...
@implementation RSCustomTextField
{
    //坐标
    CGRect   _frame;
    //用label显示内容
...
{
        if ([self.Customdelegate respondsToSelector:@selector(textViewShowDeletel:)]) {
            [self.Customdelegate textViewShowDeletel:textField];
        }
}
...
{
    // textField.text = _label.text;
    NSString *temp = [textField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    if ([temp length]!=0) {
        if ([self.Customdelegate respondsToSelector:@selector(textViewShowAndHidden:)]) {
            [self.Customdelegate textViewShowAndHidden:textField];
        }
    }
    [textField resignFirstResponder];
}
