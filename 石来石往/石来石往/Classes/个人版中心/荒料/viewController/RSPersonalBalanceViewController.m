//
//  RSPersonalBalanceViewController.m
//  石来石往
//
...
@interface RSPersonalBalanceViewController ()
...
@implementation RSPersonalBalanceViewController
    ...
    [network getDataWithUrlString:URL_BLBALANCE_IOS withParameters:parameters withBlock:^(id json, BOOL success) {
        if (success) {
            ...
            if (isresult) {
                ...
                _numberDetailLabel.text =  [NSString stringWithFormat:@"%ld颗",(long)[json[@"data"][@"total"][@"totalQty"] integerValue] ];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //cell.selectFunctionType = self.selectFunctionType;
    //码单
...
//   personalWkVc.whsId = [[self.blockDict objectForKey:@"whsId"] integerValue];
    ...
    NSString * reportUrlStr = [NSString stringWithFormat:@"%@?appType=%ld&userId=%@&VerifyKey=%@&pageType=%ld&totalQty=%ld&totalVaQty=%@&dateFrom=%@&dateTo=%@&mtlName=%@&mtlId=%ld&blockNo=%@&whsName=%@&storageType=%@&whsId=%ld&isFrozen=%ld&lengthType=%ld&widthType=%ld&length=%@&width=%@&companyName=%@&pwmsUserId=%ld", personalWkVc.webStr,(long)personalWkVc.appType,self.usermodel.userID,personalWkVc.VerifyKey, (long)personalWkVc.pageType,(long)balancemodel.qty, balancemodel.volume,personalWkVc.dateFrom,personalWkVc.dateTo,personalWkVc.mtlName,(long)personalWkVc.mtlId,personalWkVc.blockNo,personalWkVc.whsName,personalWkVc.storageType,(long)personalWkVc.whsId,(long)personalWkVc.isFrozen,(long)personalWkVc.lengthType,(long)personalWkVc.widthType,personalWkVc.length, personalWkVc.width,self.usermodel.pwmsUser.companyName,self.usermodel.pwmsUser.parentId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RSBalanceDetialViewController * balanceDetailVc = [[RSBalanceDetialViewController alloc]init];
    //[self.blockDict setObject:[NSNumber numberWithInteger:1] forKey:@"pageNum"];
...
//    [self.blockDict setValue:[self showDisplayTheTime:warehousingTime] forKey:@"dateFrom"];
//    [self.blockDict setValue:[self showDisplayTheTime:deadlingTime] forKey:@"dateTo"];
//    [self.blockDict setValue:materialName forKey:@"mtlName"];
//    [self.blockDict setValue:blockNumber forKey:@"blcokNo"];
//    [self reloadPersonalBlockNewData];
//}
...
//    [self.blockDict setObject:[self showDisplayTheTime:beginTime] forKey:@"dateFrom"];
//    [self.blockDict setObject:[self showDisplayTheTime:endTime] forKey:@"dateTo"];
//    NSArray * styleArray = @[@"采购入库",@"加工入库",@"盘盈入库"];
...
//        RSPersonlPublishDB * db = [[RSPersonlPublishDB alloc]initWithCreatTypeList:@"Warehouselist.sqlite"];
//        NSMutableArray * warehouseArray = [db getAllContent:@"BL"];
//        RSWarehouseModel * warehousemodel = warehouseArray[secondIndex];
...
//
//    NSString * str = [NSString string];
//    if ([styleArray[index] isEqualToString:@"采购入库"]) {
//        str = @"CGRK";
//    }else if ([styleArray[index] isEqualToString:@"加工入库"]){
//         str = @"JGRK";
//    }else if ([styleArray[index] isEqualToString:@"盈亏入库"]){
//        str = @"PYRK";
//    }
//
//    [self.blockDict setObject:str forKey:@"storageType"];
