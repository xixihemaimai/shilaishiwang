//
//  RSBLSpotShowCompleteViewController.m
//  石来石往
//
...
@interface RSBLSpotShowCompleteViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation RSBLSpotShowCompleteViewController
...
//    self.tableview.contentInset = UIEdgeInsetsMake(0, 0, 50, 0);
    self.tableview.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [weakSelf reloadBLCompleteSpotNewData];
        
    }];
    //向上刷新
    self.tableview.mj_footer = [MJRefreshAutoFooter footerWithRefreshingBlock:^{
        [weakSelf.BLSpotDict setObject:[NSNumber numberWithInteger:1] forKey:@"isPublic"];
        [weakSelf reloadBLCompleteSpotMoreNewData];
    }];
...
//        [weakSelf reloadBLCompleteSpotNewData];
    ...
    [self reloadBLCompleteSpotNewData];
    [self setBottomUI];
}
...
- (void)reloadBLCompleteSpotMoreNewData{
...
- (void)showSelectAction:(UIButton *)showBtn{
    ...
    if (self.isReSelect == true) {
        ...
        [network getDataWithUrlString:URL_PUBLICSTOCK_IOS withParameters:parameters withBlock:^(id json, BOOL success) {
            if (success) {
                ...
                if (isresult) {
                    ...
                    if ([weakSelf.delegate respondsToSelector:@selector(cancelshowBlockNoNewData)]) {
                        [weakSelf.delegate cancelshowBlockNoNewData];
                    }
                }else{
                    [SVProgressHUD showInfoWithStatus:@"取消展示错误"];
                }
            }else{
                [SVProgressHUD showInfoWithStatus:@"取消展示错误"];
            }
        ...
        if (self.selectArray.count > 0) {
            ...
            [SVProgressHUD showInfoWithStatus:@"你没有选择要取消展示的数据"];
        }
    }
    }else{
