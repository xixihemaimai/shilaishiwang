...
@interface RSReportFormMenuView()
...
@implementation RSReportFormMenuView

+(instancetype)MenuViewWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    
    RSReportFormMenuView * menu = [[RSReportFormMenuView alloc]initWithDependencyView:dependencyView MenuView:leftmenuView isShowCoverView:isCover];
...
//    UIPanGestureRecognizer * leftEdgeGesture = [[UIPanGestureRecognizer alloc]initWithTarget:self  action:@selector(handleLeftEdgeGesture:)];
...
-(void)show{
    ...
    //-self.menuViewframe.size.width
    self.leftMenuView.frame = CGRectMake(91, self.menuViewframe.origin.y, self.menuViewframe.size.width, self.menuViewframe.size.height);
...
#pragma mark - 屏幕往左滑处理
- (void)handleLeftEdgeGesture:(UIPanGestureRecognizer *)gesture{
    ...
    if(UIGestureRecognizerStateBegan == gesture.state ||
       UIGestureRecognizerStateChanged == gesture.state){
        //改成向左滑
        if(translation.x < SCW - 91){
            CGFloat x = translation.x + (SCW - 91);
            ...
            self.coverView.frame = CGRectMake(translation.x + 91, 0,kSCREEN_WIDTH - translation.x - 91, self.menuViewframe.size.height);
        ...
        //结束
        if(translation.x < kSCREEN_WIDTH/2){
            // 展开设置
            [self openMenuView];
        }else{
            // 恢复设置
...
-(void)handleftPan:(UIPanGestureRecognizer*)recognizer{
    ...
    if(UIGestureRecognizerStateBegan == recognizer.state ||
       UIGestureRecognizerStateChanged == recognizer.state){
        ...
        //CGFloat x           = 0 - Place;
        CGFloat x =  translation.x + 91;
        CGFloat y = self.menuViewframe.origin.y;
        CGFloat w = self.menuViewframe.size.width;
        CGFloat h = self.menuViewframe.size.height;
        self.leftMenuView.frame = CGRectMake(x, y, w, h);
        
        self.coverView.frame = CGRectMake(translation.x, 0,kSCREEN_WIDTH - translation.x, self.menuViewframe.size.height);
...
-(void)coverTap{
    ...
    [UIView animateKeyframesWithDuration:0.3 delay:0 options:UIViewKeyframeAnimationOptionLayoutSubviews animations:^{
        self.leftMenuView.frame = CGRectMake(kSCREEN_WIDTH + 91, 0, self.menuViewframe.size.width, self.menuViewframe.size.height);
...
-(void)removeCoverAndMenuView{
    ...
    self.leftMenuView.frame = CGRectMake(kSCREEN_WIDTH + 91, 0, self.leftMenuView.frame.size.width, kSCREEN_HEIGHT);
    self.coverView.frame    = CGRectMake(0, 0,kSCREEN_WIDTH, kSCREEN_HEIGHT);
    self.coverView.alpha    = 0.0;
    [self.coverView removeFromSuperview];
    [self.leftMenuView removeFromSuperview];
...
-(CGRect)menuViewframe{
    ...
    if((CGRectEqualToRect(_menuViewframe,CGRectZero)) == YES){
        ...
        //设置右侧菜单
        // kSCREEN_WIDTH * MenuView_scale_of_Screen
        CGFloat w                   = kSCREEN_WIDTH - 91;
...
-(CGRect)coverViewframe{
    ...
    if((CGRectEqualToRect(_coverViewframe, CGRectZero)) == YES){
        //self.menuViewframe.size.width
        _coverViewframe = CGRectMake(0, self.menuViewframe.origin.y, kSCREEN_WIDTH - self.menuViewframe.size.width, self.menuViewframe.size.height);
    }
