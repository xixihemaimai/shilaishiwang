...
@implementation RSSLStatementCell
...
- (void)setBalancemodel:(RSBalanceModel *)balancemodel{
    ...
    if (balancemodel.blockNo == NULL) {
        _productNameNumberLabel.hidden = YES;
    }else{
        _productNameNumberLabel.hidden = NO;
        _productNameNumberLabel.text = balancemodel.blockNo;
    }
...
//    }else{
        _productDetailNumberLabel.text = [NSString stringWithFormat:@"%@匝",balancemodel.totalTurnsQty];
//    }
   
    
    _productDetailAreaLabel.text = [NSString stringWithFormat:@"%ld片",balancemodel.totalQty];
    _productDetailWightLabel.text = [NSString stringWithFormat:@"%0.3lfm²",[balancemodel.totalVaQty doubleValue]];
}
