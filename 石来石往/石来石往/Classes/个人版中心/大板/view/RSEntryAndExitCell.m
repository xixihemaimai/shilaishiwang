...
@implementation RSEntryAndExitCell
...
- (void)setEntryandexitmodel:(RSEntryAndExitModel *)entryandexitmodel{
    _entryandexitmodel = entryandexitmodel;
    if (_entryandexitmodel.mtlName != nil) {
        _productNameLabel.text = entryandexitmodel.mtlName;
    }else{
        _productNameLabel.text = entryandexitmodel.whsName;
    }
    
    if (entryandexitmodel.blockNo == NULL) {
        _productNameNumberLabel.hidden = YES;
    }else{
        _productNameNumberLabel.hidden = NO;
        _productNameNumberLabel.text = entryandexitmodel.blockNo;
    }
    _productNumberLabel.text = @"总匝数";
    _productAreaLabel.text = @"总片数";
    _productWightLabel.text = @"总面积";
    
    //    if (balancemodel.totalTurnsQty == NULL) {
    //
    //         _productDetailNumberLabel.text = [NSString stringWithFormat:@"%@匝",balancemodel.turnsNo];
    //    }else{
    _productDetailNumberLabel.text = [NSString stringWithFormat:@"%ld匝",entryandexitmodel.turnsQty];
    //    }
    
    
    _productDetailAreaLabel.text = [NSString stringWithFormat:@"%ld片",entryandexitmodel.qty];
    _productDetailWightLabel.text = [NSString stringWithFormat:@"%0.3lfm²",[entryandexitmodel.area doubleValue]];
