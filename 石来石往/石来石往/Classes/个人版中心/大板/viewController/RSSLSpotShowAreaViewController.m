//
//  RSSLSpotShowAreaViewController.m
//  石来石往
//
...
@interface RSSLSpotShowAreaViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation RSSLSpotShowAreaViewController
...
//    self.tableview.contentInset = UIEdgeInsetsMake(0, 0, 50, 0);
    ...
    self.tableview.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [weakSelf reloadSLSpotNewData];
    }];
    //向上刷新
    self.tableview.mj_footer = [MJRefreshAutoFooter footerWithRefreshingBlock:^{
        [weakSelf.BLSpotDict setObject:[NSNumber numberWithInteger:0] forKey:@"isPublic"];
        [weakSelf reloadSLSpotMoreNewData];
    }];
        ...
        [network getDataWithUrlString:URL_PUBLICSTOCK_IOS withParameters:parameters withBlock:^(id json, BOOL success) {
            if (success) {
                ...
                if (isresult) {
                    ...
                    [weakSelf.selectArray removeAllObjects];
                    weakSelf.allBtn.selected = false;
                    if ([weakSelf.delegate respondsToSelector:@selector(showSLBlockNoNewData)]) {
                        [weakSelf.delegate showSLBlockNoNewData];
                    }
                }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.isReSelect == true) {
        ...
        self.totalContentLabel.text = [NSString stringWithFormat:@"%ld/%ld",(long)self.totalcount,(long)self.totalcount - (long)self.selectArray.count];
    }else{
        if (self.selectArray.count > 0) {
            //当再次选择搜索的时候
            for (int j = 0; j < self.selectArray.count; j++) {
