//
//  RSSLAllSelectSegmentViewController.m
//  石来石往
//
...
#import "RSSLSpotShowAreaViewController.h"
#import "RSSLSpotShowCompleteViewController.h"

@interface RSSLAllSelectSegmentViewController ()<UIScrollViewDelegate,RSSLSpotShowAreaViewControllerDelegate,RSSLSpotShowCompleteViewControllerDelegate>
...
@implementation RSSLAllSelectSegmentViewController
...
- (void)viewDidLoad {
    ...
    reportformView.selectFunctionType =@"大板库存余额表";
    RSWeakself
...
//        }
        if ([longTypeView isEqualToString:@"请选择类型"]) {
            ...
            [weakSelf.BLSpotDict setObject:[NSNumber numberWithInteger:longIndex] forKey:@"lengthType"];
        }
        
        if ([withTypeView isEqualToString:@"请选择类型"]) {
            [weakSelf.BLSpotDict setObject:[NSNumber numberWithInteger:0] forKey:@"widthType"];
        }else{
            [weakSelf.BLSpotDict setObject:[NSNumber numberWithInteger:withIndex] forKey:@"widthType"];
        }
        
        if ([turnNoStr isEqualToString:@""]) {
            
            [self.BLSpotDict setObject:@"" forKey:@"turnsNo"];
        }else{
            [self.BLSpotDict setObject:turnNoStr forKey:@"turnsNo"];
        }
        
        if ([slNoStr isEqualToString:@""]) {
            
            [self.BLSpotDict setObject:@"" forKey:@"slNo"];
            
        }else{
            
            [self.BLSpotDict setObject:slNoStr forKey:@"slNo"];
        }
        ...
        for (int i = 0; i < self.childViewControllers.count; i++) {
            if (i == 0) {
                 RSSLSpotShowAreaViewController * vc = (RSSLSpotShowAreaViewController *)weakSelf.childViewControllers[i];
                ...
                RSSLSpotShowCompleteViewController * vc = (RSSLSpotShowCompleteViewController *)weakSelf.childViewControllers[i];
                vc.BLSpotDict = weakSelf.BLSpotDict;
                [vc.BLSpotDict setObject:[NSNumber numberWithInteger:1] forKey:@"pageNum"];
                ...
                vc.delegate = self;
                vc.allBtn.selected = false;
                [vc reloadSLCompleteSpotNewData];
            }
        }
...
{
    // 未展示
    RSSLSpotShowAreaViewController *DramaVc = [[RSSLSpotShowAreaViewController alloc] init];
    ...
    //已展示
    RSSLSpotShowCompleteViewController *recommendVc = [[RSSLSpotShowCompleteViewController alloc] init];
...
- (void)showSLBlockNoNewData{
    for (int i = 0; i < self.childViewControllers.count; i++) {
        if (i == 1) {
            RSSLSpotShowCompleteViewController * vc = (RSSLSpotShowCompleteViewController *)self.childViewControllers[i];
...
- (void)cancelSLShowBlockNewData{
    for (int i = 0; i < self.childViewControllers.count; i++) {
        if (i == 0) {
            RSSLSpotShowAreaViewController * vc = (RSSLSpotShowAreaViewController *)self.childViewControllers[i];
            vc.BLSpotDict = self.BLSpotDict;
            vc.delegate = self;
            vc.type = @"DETAIL";
            [vc.selectArray removeAllObjects];
