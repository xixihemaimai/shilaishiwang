//
//  RSSLDetailStatementViewController.m
//  石来石往
//
...
@interface RSSLDetailStatementViewController ()
{
...
@implementation RSSLDetailStatementViewController
        ...
        if (success) {
            ...
            if (isresult) {
                ...
                for (int i = 0; i < array.count; i++) {
                    ...
                    [weakSelf.detialArray addObject:selectDabanslmodel];
                }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    RSSelectDabanSLModel * selectdabanslmodel = self.detialArray[section];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    RSSelectDabanSLModel * selectddabanslmodel = self.detialArray[section];
    if ([self.expendArray containsObject:@(section)]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    RSSelectDabanSLModel * selectdabanslmodel = self.detialArray[indexPath.section];
    ...
    if (slstoragemanagementmodel.isfrozen == 1) {
        cell.iamgeView.hidden = NO;
    }else{
...
- (void)showAndHideSelectContentAction:(UIButton *)selectBtn{
    ...
    RSSelectDabanSLModel * selectdabanslmodel = self.detialArray[cell.tag];
...
- (void)headerButtonOnClick:(UIButton *)button {
    RSSelectDabanSLModel * selectdabanslmodel = self.detialArray[button.tag];
