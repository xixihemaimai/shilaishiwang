//
//  RSSLEntryAndExitViewController.m
//  石来石往
//
...
#import "RSEntryAndExitCell.h"
#import "RSSLEnterDetailViewController.h"
@interface RSSLEntryAndExitViewController ()
{
...
@implementation RSSLEntryAndExitViewController
...
- (void)viewDidLoad {
    ...
    reportformView.reportformCreateUserNameSLSelect = ^(NSString * _Nonnull inSelect, NSString * _Nonnull beginTime, NSString * _Nonnull endTime, NSString * _Nonnull wuliaoTextView, NSString * _Nonnull blockTextView, NSString * _Nonnull longTypeView, NSInteger longIndex, NSString * _Nonnull longTextView, NSString * _Nonnull withTypeView, NSInteger withIndex, NSString * _Nonnull withTextView, NSString * _Nonnull luTypeView, NSString * _Nonnull wareHouseView, NSInteger wareHouseIndex, NSString * _Nonnull frozenView, NSInteger frozenviewIndex, NSString * _Nonnull turnNoStr, NSString * _Nonnull slNoStr, NSString * _Nonnull createUserName) {
...
//
        ...
        if ([self.selectFunctionType isEqualToString:@"大板出库明细表"]) {
        ...
        if (success) {
            ...
            if (isresult) {
                ...
                for (int i = 0; i < array.count; i++) {
                    RSEntryAndExitModel * entryandexitmodel = [[RSEntryAndExitModel alloc]init];
                    entryandexitmodel.area = [[array objectAtIndex:i]objectForKey:@"area"];
                    entryandexitmodel.qty = [[[array objectAtIndex:i]objectForKey:@"qty"]integerValue];
                    entryandexitmodel.turnsQty = [[[array objectAtIndex:i]objectForKey:@"turnsQty"]integerValue];
                    entryandexitmodel.whsId =  [[[array objectAtIndex:i]objectForKey:@"whsId"]integerValue];
                    entryandexitmodel.mtlId =  [[[array objectAtIndex:i]objectForKey:@"mtlId"]integerValue];
                      entryandexitmodel.mtlName = [[array objectAtIndex:i]objectForKey:@"mtlName"];
                    entryandexitmodel.blockNo = [[array objectAtIndex:i]objectForKey:@"blockNo"];
                    entryandexitmodel.whsName = [[array objectAtIndex:i]objectForKey:@"whsName"];
                    [weakSelf.balanceArray addObject:entryandexitmodel];
                }
        ...
        if (success) {
            ...
            if (isresult) {
                ...
                for (int i = 0; i < array.count; i++) {
                    ...
                    entryandexitmodel.blockNo = [[array objectAtIndex:i]objectForKey:@"blockNo"];
                    entryandexitmodel.whsName = [[array objectAtIndex:i]objectForKey:@"whsName"];
                    [tempArray addObject:entryandexitmodel];
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString * PERSONALBALANCECELLID = @"ENTRYANDEXITCELLID";
    RSEntryAndExitCell * cell = [tableView dequeueReusableCellWithIdentifier:PERSONALBALANCECELLID];
    if (!cell ) {
        cell = [[RSEntryAndExitCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:PERSONALBALANCECELLID];
    }
    cell.entryandexitmodel = self.balanceArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RSSLEnterDetailViewController * balanceDetailVc = [[RSSLEnterDetailViewController alloc]init];
    RSEntryAndExitModel * entryandexitmodel = self.balanceArray[indexPath.row];
    ...
    [dict setObject:[self.blockDict objectForKey:@"dateTo"] forKey:@"dateTo"];
     if ([self.type isEqualToString:@"SUM"]) {
         [dict setObject:entryandexitmodel.blockNo forKey:@"blockNo"];
     }else{
    ...
    if ([self.type isEqualToString:@"SUM"]) {
        ...
        [dict setObject:[NSNumber numberWithInteger:entryandexitmodel.mtlId] forKey:@"mtlId"];
        [dict setObject:[self.blockDict objectForKey:@"whsId"] forKey:@"whsId"];
        [dict setObject:entryandexitmodel.mtlName forKey:@"mtlName"];
        ...
        [dict setObject:entryandexitmodel.whsName forKey:@"whsName"];
        [dict setObject:[NSNumber numberWithInteger:entryandexitmodel.whsId] forKey:@"whsId"];
    }
...
- (void)codeSendAction:(UIButton *)codeSheetBtn{
    RSEntryAndExitModel * entryandexitmodel = self.balanceArray[codeSheetBtn.tag];
...
//    personalWkVc.mtlId = [[self.blockDict objectForKey:@"mtlId"]integerValue];
    ...
    if ([self.type isEqualToString:@"SUM"]) {
        personalWkVc.blockNo = entryandexitmodel.blockNo;
        personalWkVc.mtlName = entryandexitmodel.mtlName;
        personalWkVc.mtlId =  [[self.blockDict objectForKey:@"mtlId"]integerValue];
        ...
        personalWkVc.mtlName = [self.blockDict objectForKey:@"mtlName"];
        personalWkVc.mtlId = [[self.blockDict objectForKey:@"mtlId"]integerValue];
        personalWkVc.whsId = entryandexitmodel.whsId;
        personalWkVc.whsName = entryandexitmodel.whsName;
...
//    NSRange range = [_numberDetailLabel.text rangeOfString:@"匝"];//匹配得到的下标
//    string = [_numberDetailLabel.text substringToIndex:range.location];
//
//    NSString * twostring = [NSString string];
//    NSRange range1 = [_totalVolmueDetailLabel.text rangeOfString:@"m²"];//匹配得到的下标
//    twostring = [_totalVolmueDetailLabel.text substringToIndex:range1.location];
    ...
    NSString * reportUrlStr = [NSString stringWithFormat:@"%@?appType=%ld&userId=%@&VerifyKey=%@&pageType=%ld&totalTurnsQty=%ld&totalVaQty=%@&totalQty=%ld&dateFrom=%@&dateTo=%@&mtlName=%@&mtlId=%ld&blockNo=%@&whsName=%@&storageType=%@&whsId=%ld&isFrozen=%ld&lengthType=%ld&widthType=%ld&length=%@&width=%@&turnsNo=%@&slNo=%@&companyName=%@&pwmsUserId=%ld", personalWkVc.webStr,personalWkVc.appType,self.usermodel.userID,personalWkVc.VerifyKey, personalWkVc.pageType,entryandexitmodel.turnsQty, entryandexitmodel.area,entryandexitmodel.qty,personalWkVc.dateFrom,personalWkVc.dateTo,personalWkVc.mtlName,personalWkVc.mtlId,personalWkVc.blockNo,personalWkVc.whsName,personalWkVc.storageType,personalWkVc.whsId,personalWkVc.isFrozen,personalWkVc.lengthType,personalWkVc.widthType,personalWkVc.length, personalWkVc.width,personalWkVc.turnsNo,personalWkVc.slNo,self.usermodel.pwmsUser.companyName,self.usermodel.pwmsUser.parentId];
...
- (void)reportFromSendAction:(UIButton *)reportFormBtn{
    RSEntryAndExitModel * entryandexitmodel = self.balanceArray[reportFormBtn.tag];
    RSPersonalWkWebviewViewController * personalWkVc = [[RSPersonalWkWebviewViewController alloc]init];
    personalWkVc.title = self.selectFunctionType;
...
//    }
  ...
  //  personalWkVc.whsName = [self.blockDict objectForKey:@"whsName"];
   ...
   // personalWkVc.whsId = [[self.blockDict objectForKey:@"whsId"] integerValue];
    ...
    NSUserDefaults * user = [NSUserDefaults standardUserDefaults];
    personalWkVc.VerifyKey = [user objectForKey:@"VERIFYKEY"];
    // personalWkVc.totalQty = balancemodel.totalTurnsQty;
