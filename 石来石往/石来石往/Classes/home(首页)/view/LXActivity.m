...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray andRSFriend:(RSFriendModel *)friendmodel;
{
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor colorWithRed:142/255.0 green:142/255.0 blue:142/255.0 alpha:0.5];
...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray andTaobaoUserListModel:(RSTaoBaoUserLikeModel *)taobaoUserListmodel;
{
    ...
    if (self) {
        ...
        self.taobaoUserListmodel = taobaoUserListmodel;
        if (delegate) {
            self.delegate = delegate;
        }
...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray andRSCodeSheetModel:(RSCodeSheetModel *)codeSheetModel{
    ...
    if (self) {
        ...
        self.codeSheetModel = codeSheetModel;
        if (delegate) {
            self.delegate = delegate;
        }
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    ...
    self.backGroundView.backgroundColor = [UIColor colorWithHexColorStr:@"#EAEFF3"];
    ...
    if (shareButtonImagesNameArray) {
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                [shareButton setBackgroundImage:[UIImage imageNamed:@"Rectangle 18"] forState:UIControlStateNormal];
                [shareButton setImage:[UIImage imageNamed:[shareButtonImagesNameArray objectAtIndex:i - 1]] forState:UIControlStateNormal];
                shareButton.imageEdgeInsets = UIEdgeInsetsMake(14, 14, 14, 14);
                //有Title的时候
                if (self.isHadTitle == YES) {
                    if (i == 1) {
                        [shareButton setFrame:CGRectMake(99, self.LXActivityHeight+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), 60,60)];
                    }else if (i == 2){
                          [shareButton setFrame:CGRectMake(CGRectGetMaxX(shareButton.frame) + 55, self.LXActivityHeight+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), 60, 60)];
                    }else{
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                if (self.isHadTitle == YES) {
                    if (j == 1) {
                         [shareLabel setFrame:CGRectMake(99, self.LXActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)) + 20, 60, SHARETITLE_HEIGHT)];
                    }else if (j == 2){
                         [shareLabel setFrame:CGRectMake(CGRectGetMaxX(shareLabel.frame) + 55, self.LXActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)) + 20, 60, SHARETITLE_HEIGHT)];
                    }else{
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
            [cancelButton setFrame:CGRectMake(0, self.LXActivityHeight + 20,SCW, cancelButton.frame.size.height)];
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(0, BUTTON_INTERVAL_HEIGHT, SCW, BUTTON_HEIGHT)];
   ...
   // cancelButton.layer.borderWidth = BUTTON_BORDER_WIDTH;
    ...
    [cancelButton setTitleColor:[UIColor colorWithHexColorStr:@"#666666"] forState:UIControlStateNormal];
...
- (UILabel *)creatShareLabelWithColumn:(int)column andLine:(int)line
{
    ...
    shareLabel.textColor = [UIColor colorWithHexColorStr:@"#666666"];
    return shareLabel;
}

- (UILabel *)creatTitleLabelWith:(NSString *)title
{
    UILabel *titlelabel = [[UILabel alloc] initWithFrame:CGRectMake(0, TITLE_INTERVAL_HEIGHT, SCW, TITLE_HEIGHT)];
...
- (void)didClickOnImageIndex:(UIButton *)button
{
    if (self.delegate) {
        if (button.tag != self.postionIndexNumber-1) {
            ...
            //这边要区分开来
            if (self.codeSheetModel != nil) {
                
                if ([self.delegate respondsToSelector:@selector(didClickOnImageIndex:andRSCodeSheetModel:)] == YES) {
                    [self.delegate didClickOnImageIndex:(NSInteger *)button.tag andRSCodeSheetModel:self.codeSheetModel];
                }
            }else if(self.friendmodel != nil){
                if ([self.delegate respondsToSelector:@selector(didClickOnImageIndex:andRSFriendModel:)] == YES) {
                    [self.delegate didClickOnImageIndex:(NSInteger *)button.tag andRSFriendModel:self.friendmodel];
                }
            }else if (self.moment != nil){
                if ([self.delegate respondsToSelector:@selector(didClickOnImageIndex:andMoment:)]) {
                    [self.delegate didClickOnImageIndex:(NSInteger *)button.tag andMoment:self.moment];
                }
                
            }else if (self.taobaoUserListmodel != nil){
                if ([self.delegate respondsToSelector:@selector(didClickOnmageIndex:andTaobaoShare:)]) {
                    //[self.delegate didClickOnTaobaoShare:self.taobaoUserListmodel];
                    [self.delegate didClickOnmageIndex:(NSInteger *)button.tag andTaobaoShare:self.taobaoUserListmodel];
                }
            }
        }
        else{
