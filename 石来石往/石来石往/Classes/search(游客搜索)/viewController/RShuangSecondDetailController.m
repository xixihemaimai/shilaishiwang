//
//  RShuangSecondDetailController.m
//  石来石往
//
...
#define ECA 2
#define margin 20
@interface RShuangSecondDetailController ()<DOPDropDownMenuDelegate,DOPDropDownMenuDataSource,UITableViewDelegate,UITableViewDataSource,RSLeftScreenViewDelegate,RSLeftFunctionScreenViewDelegate,RSEmptyDataViewDelegate>
...
@implementation RShuangSecondDetailController
...
//- (RSEmptyDataView *)emptyDataView{
//    if (!_emptyDataView){
//        _emptyDataView = [[RSEmptyDataView alloc]initWithFrame:CGRectMake(0, -130, SCW, 160)];
//        [self.view addSubview:_emptyDataView];
//    }
//    return _emptyDataView;
//}

//- (RSShowKeywordView *)showKeywordView{
//    if (!_showKeywordView) {
//        _showKeywordView = [[RSShowKeywordView alloc]initWithFrame:CGRectMake(0, -130, SCW, 160)];
//        //self.showKeywordView.frame = CGRectMake(0, CGRectGetMaxY(self.emptyDataView.frame) + 10, SCW, 100);
//        [self.view addSubview:_showKeywordView];
//    }
//    return _showKeywordView;
//}
...
- (void)viewDidLoad {
    ...
    //
    //_emptyDataView.array = array;
    
    
    //[self.emptyDataView addKeywordArray:array];
    ...
    [self loadHuangliaoMoreMarketNewData];
...
- (void)setUI{
    ...
    for (int i = 0; i < self.marketArray.count; i++) {
        ...
        if (i == self.selectIndex) {
            ...
            //            choiceMarketview.numeberLabel.text = @"233445m³";
            //            choiceMarketview.marketLabel.text = @"海西石材城";
        }else{
    ...
    //  self.tableview.tableHeaderView = marketScrollview;
    ...
    RSLeftScreenView *showRightview = [[RSLeftScreenView alloc]initWithFrame:CGRectMake(0, 42, 268,_leftview.bounds.size.height)];
    ...
    for (RSChoiceMarketView * choiceMarketview in self.marketScrollview.subviews) {
        for (UIButton * choiceBtn in choiceMarketview.subviews) {
            if (choiceBtn.tag == btn.tag) {
                ...
                RSHuangAndDabanMarketModel * huangAndDabanmarketmodel =  [self.marketArray objectAtIndex:btn.tag];
...
- (void)loadHuangliaoMoreMarketNewData{
    ...
    [dict setObject:@"huangliao" forKey:@"search_type"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    [btn addTarget:self action:@selector(jumpHuangliaoContent:) forControlEvents:UIControlEventTouchUpInside];
    return segmentview;
}

- (void)jumpHuangliaoContent:(UIButton *)btn{
    /*
     跳转俩种界面
     */
